...
export async function generatePreviewWithWatermarkSafe(
  ...
  try {
    ...
    // Replace this with your actual watermark generation logic
    const watermarkedBuffer = await generatePreviewWithWatermark(buffer);

    // Return the processed buffer AND the original dimensions
    return { buffer: watermarkedBuffer, width: metadata.width, height: metadata.height };
  } catch (error) {
...
const CLEAN_PREVIEW_MAX_WIDTH = 1920; // Can be larger for detail pages

export async function generateCleanPreviewSafe(
  buffer: Buffer
): Promise<{ buffer: Buffer; width: number; height: number } | null> {
  try {
    const image = sharp(buffer, {
    ...
    // Just resize, no watermarking
    const resizedImage = image
      .rotate() // Auto-rotate based on EXIF data
      .resize({
        width: CLEAN_PREVIEW_MAX_WIDTH,
        fit: 'inside',
        withoutEnlargement: true,
      });
    ...
    // --- MODIFIED: AGGRESSIVE compression settings to minimize file size ---
    switch (originalFormat) {
      case 'png':
        // Using palette is key for PNGs. quality sets the color count.
        // compressionLevel 9 is max.
        previewBuffer = await resizedImage
          .png({
            quality: 70,
            compressionLevel: 9,
            palette: true,
      ...
      case 'webp':
        // WebP is very efficient. 65 is a very aggressive quality setting.
        previewBuffer = await resizedImage
          .webp({ quality: 65 })
          .toBuffer();
        break;
      ...
      case 'jpg':
      default:
        // For JPEG, 65 is a very low quality setting.
        // `mozjpeg: true` is crucial for getting the best compression.
        previewBuffer = await resizedImage
          .jpeg({
            quality: 65,
            mozjpeg: true,
    ...
    // Return the resized buffer and ORIGINAL dimensions
    return { buffer: previewBuffer, width: metadata.width, height: metadata.height };
  } catch (error) {
    console.error('Error generating clean preview:', error);
    return null;
  }
}
...
export function getPreviewFileName(fileName: string): string {
  ...
  const ext = lastDotIndex !== -1 ? sanitized.slice(lastDotIndex) : '.jpg'; // Fallback to .jpg

  return `preview-wm-${name}${ext}`;
}
