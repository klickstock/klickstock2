...
async function getWatermarkTileBuffer(): Promise<Buffer> {
  ...
  // --- FIX 1: Removed leading newline from the SVG string ---
  // The XML declaration must be the very first thing in the string for the parser to work correctly.
  const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg id="Pattren" xmlns="http://www.w3.org/2000/svg" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 2000 2000">
  <!-- (Your full SVG content goes here... I've omitted it for brevity) -->
  ...
  watermarkTileBuffer = await sharp(Buffer.from(svg))
    .resize(TILE_SIZE, TILE_SIZE)
    .png() // The tile itself should be a PNG to support transparency
    .toBuffer();
...
/**
 * Generates a preview image by resizing the original and overlaying a watermark.
 * It now intelligently handles the output format to preserve transparency (e.g., for PNGs).
 *
 * @param imageBuffer The buffer of the original uploaded image.
 * @returns A promise that resolves to the buffer of the watermarked preview image.
 */
export async function generatePreviewWithWatermark(imageBuffer: Buffer): Promise<Buffer> {
  try {
    // Step 1: Get metadata, including the format, from the ORIGINAL buffer first.
    const originalMetadata = await sharp(imageBuffer).metadata();
    const originalFormat = originalMetadata.format;

    // Step 2: Resize the main image.
    const resizedImage = await sharp(imageBuffer, {
      failOnError: false, // More resilient to corrupted image data
    })
      .rotate() // Auto-rotate based on EXIF data
      .resize({
        width: PREVIEW_MAX_WIDTH,
        fit: 'inside', // Ensures the image fits within the bounds
        withoutEnlargement: true, // Don't scale up small images
      })
      .toBuffer();

    // Step 3: Get dimensions of the *resized* image to decide on tiling logic.
    const { width, height } = await sharp(resizedImage).metadata();
    if (!width || !height) {
      throw new Error('Failed to retrieve image dimensions after resizing.');
    }
    ...
    // Use tiling for images large enough to support it.
    if (width >= TILE_SIZE && height >= TILE_SIZE) {
      watermarkCompositeOptions = {
        input: await getWatermarkTileBuffer(),
        tile: true,
      ...
      // For small images, create a single watermark that covers the image.
      const singleWatermarkBuffer = await sharp(await getWatermarkTileBuffer())
        .resize(width, height, { fit: 'cover' }) // 'cover' ensures the pattern fills the small area
        .png()
        .toBuffer();

      watermarkCompositeOptions = {
        input: singleWatermarkBuffer,
        tile: false, // Not tiling this one
        blend: 'over',
      };
    }
    ...
    // --- FIX 2: Conditionally set the output format based on the original. ---
    // This preserves transparency for formats like PNG and WebP.
    let finalBuffer: Buffer;

    switch (originalFormat) {
      case 'png':
        finalBuffer = await watermarkedImage
          .png({ quality: 90 }) // Preserve PNG format
          .toBuffer();
        break;
      ...
      case 'jpg':
      default:
        // Default to high-quality JPEG for original JPEGs or any other unhandled format.
        finalBuffer = await watermarkedImage
          .jpeg({
            quality: 90,
...
/**
 * A safe wrapper for the watermark generation function.
